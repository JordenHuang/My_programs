...
void set_goal_state(int goal_state[][SIDE_LENGTH]){
    int i, j, num=1;
...
boool at_goal(int current_state[][SIDE_LENGTH], int goal_state[][SIDE_LENGTH]){
    int i, j;
...
void get_initial_state(int current_state[][SIDE_LENGTH]){
    int  i, j, n=0;
    ...
    while(1){
        ...
        for (i=0; i<SIDE_LENGTH; i++){
            printf("     ");
            for (j=0; j<SIDE_LENGTH; j++) printf(" %*d", pad, current_state[i][j]);
            printf("\n");
        }
...
void four_move_swap(int current_state[][SIDE_LENGTH]){
    move_RIGHT;
    move_UP;
    move_LEFT;
    move_DOWN;
...
void get_target_pos(int goal_state[][SIDE_LENGTH], int target_num, int target_pos[2]){
    int i, j;
    // printf("target num: %d\n", target_num);
    for(i=0; i<SIDE_LENGTH; i++){
        for(j=0; j<SIDE_LENGTH; j++){
...
void initialize_buffer(int current_state[][SIDE_LENGTH]){
    // printf("zero pos x: %d\n", zero_pos[0]);
...
void set_up_move(int goal_state[][SIDE_LENGTH], int current_state[][SIDE_LENGTH], int to_solve){
    // printf("to solve is: %d\n", to_solve);
    int target_pos[2], ud_bound, rl_bound;

    get_target_pos(goal_state, to_solve, target_pos);
...
void undo_set_up_move(int current_state[][SIDE_LENGTH]){
    moves undo_move;
    int target_pos[2];
...
void last_column_case(int goal_state[][SIDE_LENGTH], int current_state[][SIDE_LENGTH], int to_solve){
    int target_pos[2], ud_bound, rl_bound;
    get_target_pos(goal_state, to_solve, target_pos);
    ...
    four_move_swap(current_state);
    undo_set_up_move(current_state);
    four_move_swap(current_state);
}


void special_case_swap(int goal_state[][SIDE_LENGTH], int current_state[][SIDE_LENGTH], int to_solve){
...
void swap_with_unsolved(int goal_state[][SIDE_LENGTH], int current_state[][SIDE_LENGTH]){
    ...
    if(goal_state[(num-1)/SIDE_LENGTH][(num-1)%SIDE_LENGTH] % SIDE_LENGTH == 0){
        last_column_case(goal_state, current_state, goal_state[(num-1)/SIDE_LENGTH][(num-1)%SIDE_LENGTH]);
    }
    // else normal case
    else{
    set_up_move(goal_state, current_state, goal_state[(num-1)/SIDE_LENGTH][(num-1)%SIDE_LENGTH]);
    four_move_swap(current_state);
    undo_set_up_move(current_state);
    four_move_swap(current_state);
    }
...
void eight_puzzle_tile_3(int current_state[][SIDE_LENGTH]){
    // move right (set up)
    move_RIGHT;
...
// /*
void solver(int goal_state[][SIDE_LENGTH], int current_state[][SIDE_LENGTH]){
    int i, j;
    int to_solve;

    initialize_buffer(current_state);
    ...
    while(1){
        ...
        if((to_solve == goal_state[SIDE_LENGTH-2][SIDE_LENGTH-2]) || (to_solve == goal_state[SIDE_LENGTH-2][SIDE_LENGTH-1])){
            ...
            special_case_swap(goal_state, current_state, to_solve);
        }
        // if to_solve = 15
        else if(to_solve == SIDE_LENGTH*SIDE_LENGTH - 1){
            // all solved case
            if(at_goal(current_state, goal_state) == True){
                move_RIGHT;
                break;
            }
            // tile 15 case
                ...
                // printf("15 case\n");
                swap_with_unsolved(goal_state, current_state);
                continue;
            }
        }
        ...
        // tile 4 or tile 8 case
            ...
            if(to_solve == goal_state[SIDE_LENGTH-3][SIDE_LENGTH-1]){
                eight_puzzle_tile_3(current_state);
            }
            // for other last column cases
            else{
                last_column_case(goal_state, current_state, to_solve);
            }
        }
        else{
            // printf("normal case\n");
            set_up_move(goal_state, current_state, to_solve);
            four_move_swap(current_state);
            undo_set_up_move(current_state);
            four_move_swap(current_state);
        }
...
int main(){
    ...
    int current_state[SIDE_LENGTH][SIDE_LENGTH];


    buffer_pos[0] = SIDE_LENGTH-2;
    buffer_pos[1] = SIDE_LENGTH-2;

    set_goal_state(goal_state);
    ...
    solver(goal_state, current_state);
