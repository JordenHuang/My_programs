...
boool found_goal_state = False;
boool already_write = False;
nodes *start_state;
...
void write_to_file(){
    list *cur = list_head->next_node;
    char f_name[256], n[16];
    FILE *fpt;

    if (getcwd(f_name, sizeof(f_name)) == NULL)
        perror("getcwd() error");
    // else
    //     printf("current working directory is: %s\n", f_name);

    strcat(f_name, "\\test_1");
    CreateDirectory(f_name, NULL);

    strcat(f_name, "\\");

    sprintf(n, "%03d_%d_%d", last_node->puzzle_node->depth, last_node->puzzle_node->heuristic, total_nodes);
    strcat(f_name, n);
    strcat(f_name, ".txt");

    fpt = fopen(f_name, "w");

    // fprintf(fpt, "%d\n", last_node->puzzle_node->f);
    
    while(cur != NULL){
        switch (cur->puzzle_node->move){
            case up:
            fprintf(fpt, "U\n");
            break;
...
int search(int threshold){
    ...
    if(current->f > threshold){
        // printf(" s1.1\n");
        if(current->f >= break_time && already_write == False){
            write_to_file();
            already_write = True;
            printf("\n");
            for (i = 0; i < SIDE_LENGTH; i++){
            printf("     ");
            for (j = 0; j < SIDE_LENGTH; j++) printf(" %2d", current->state[i][j]);
            printf("\n");
        }
        }
...
void ida_star(nodes *current_node){
    ...
    while(1){
        ...
        if(threshold >= break_time){
            printf("\nwrite to file\n");
            // if(last_node == list_head) printf("true");
            return;
        }
        already_write = False;
    }
...
int main(){
    ...
    break_time = start_state->heuristic + 5;  //-----------------------------------------------
...
TODO: 測試老師給的5*5測資
TODO: maybe change the bubble sort to quick sort
*/
