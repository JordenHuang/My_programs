...
int cube_width=20;  // 方塊的邊長
int width=120, height=48;  // 寬跟高
    ...
    while(1){
        ...
        for(cube_y = 0; cube_y <= cube_width; cube_y += 0.5){
            for(cube_x = 0; cube_x <= cube_width; cube_x += 0.5){
                calculate_cube_surface(cube_x, cube_y, -cube_width, '+');  // 內部
                ...
                calculate_cube_surface(cube_x, cube_y, -cube_width*2, '-');  // 外圍
