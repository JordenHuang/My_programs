...
#   set up hoster and wait for user to connect
hoster_ip = socket.gethostbyname(socket.gethostname())
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
sock.bind((hoster_ip, 55655))
sock.listen(1)
print('hoster ip:', hoster_ip)
...
then you\'ll have to copy the file to the same directory with this program inside!"\n')
    ...
    def get_file_size(file_name, file):
        ...
        user.send(file_name_and_size.encode('utf-8'))
...
def receiver():
    ...
    file_name_and_size = user.recv(1024)
